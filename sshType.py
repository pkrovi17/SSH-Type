...
def monkeytype(stdscr):
    ...
    while True:
        stdscr.clear()
        stdscr.addstr(0, 0, f"Time's up! ")
        stdscr.addstr(1, 0, f"Final WPM: {wpm_val}")
        stdscr.addstr(2, 0, f"Errors: {errors}")
        stdscr.addstr(3, 0, f"Accuracy: {accuracy}%")
        stdscr.addstr(4, 0, f"Consistency (std dev): ±{consistency} WPM")
        stdscr.addstr(6, 0, 'Type "quit" to exit or press Ctrl+C')
        stdscr.addstr(8, 0, f"> {typed_exit}")
        stdscr.refresh()
